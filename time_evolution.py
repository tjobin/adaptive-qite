...
# I, Timothé Jobin, have modified the original file.
...
from qiskit.circuit import Parameter  # Modif
...
def _build_aux_operators(
    ...
    Args:
        ...
        i: the i-th timestep (matters now, since all timesteps are different)
...
def _build_step_operator(
        evolution_problem, real_time, timestep
    ) -> csr_matrix:
...
def _evolve(
    evolution_problem: TimeEvolutionProblem, num_timesteps: int, real_time: bool, params, filename=None
) -> TimeEvolutionResult:
    ...
    for ts in range(num_timesteps):  # pylint: disable=invalid-name
        dtau_ts = params[ts]
        aux_ops = _build_aux_operators(
        evolution_problem=evolution_problem
        )   
        step_operator = _build_step_operator(evolution_problem, False, dtau_ts)

        ops_ev_mean[:, ts] = _evaluate_aux_ops(aux_ops, state)
        state = expm_multiply(A=step_operator, B=state)
        state = renormalize(state)
        # if ops_ev_mean[-1, ts] > 0:
        # print(ops_ev_mean[:,ts])
        if filename is not None:
            time_counter += dtau_ts
            # fout.write('check')
            fout.write("%.0f %.6f %.6f \n" % (ts, time_counter, ops_ev_mean[-1, ts]))
